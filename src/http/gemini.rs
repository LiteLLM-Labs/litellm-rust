...
/// Authorize a native Gemini request. Gemini callers may authenticate via
/// `x-goog-api-key` or `?key=` (in addition to the standard gateway headers), so
/// accept those here before falling back to the shared gateway-key check.
fn authorize(
    state: &AppState,
    headers: &HeaderMap,
